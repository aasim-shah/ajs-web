...
import { fetchChats, fetchMessages, sendMessage, receiveMessage } from "@/store/slices/messageSlice";
...
const MessageList = () => {
  ...
  const { chats, status, error } = useSelector((state: RootState) => state.messageSlice);
  ...
  const handleSendMessage = () => {
    if (selectedChat && newMessage.trim() !== "" && accessToken) {
      ...
      const messageToSend = {
        sender: userId, // Make sure to include the sender ID
      ...
      dispatch(sendMessage(messageToSend))
        .unwrap()
        .then(() => {
          setMessages((prevMessages) => [...prevMessages, messageToSend]); // Update the messages state
          setNewMessage("");
          if (inputRef.current) {
  ...
  const getUserProfilePicture = (user: User | undefined) => {
    if (!user) return '/images/placeholderimage.png';
    const profilePicture = user.profilePicture || user.companyLogo || '/images/placeholderimage.png';
  ...
  {status === "loading" ? (
    <div className="flex flex-col items-center justify-center h-full">
      <p className="text-lg text-signature opacity-70">Loading Messages...</p>
    </div>
  ) : messages.length > 0 ? (
    <ul>
      {messages.map((msg) => {
        ...
        const chat = chats.find((chat: Chat) => chat._id === selectedChat);
        const displayUser = getMessageUser(chat?.users, msg.sender);
          ...
          <li key={msg._id} className={`mb-4 flex ${isSender ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex items-center ${isSender ? 'flex-row-reverse' : ''}`}>
              <Image
                ...
                onError={(e) => (e.currentTarget.src = '/images/placeholderimage.png')}
              />
              <div className={`p-2 rounded-lg ${isSender ? 'bg-signature text-background' : 'bg-gray-200 text-foreground'}`}>
                <p>{msg.message}</p>
                <span className="text-xs text-foreground">{new Date(msg.createdAt).toLocaleTimeString()}</span>
              </div>
            </div>
          </li>
        );
